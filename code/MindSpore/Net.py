...
class CLIPAlpha(nn.Module):
    def __init__(self):
        super().__init__()
        self.clip_model, self.clip_preprocess = clip.load("ViT-B/32", device="npu")
        self.text = clip.tokenize(["A photo of high quality", "A photo of low quality"]).npu()
    ...
    def forward(self, x, batch_size,p):
        ...
        i = 0
        clip_score = torch.zeros([batch_size, 1]).npu()
        for clip_img in x:
            if p[i,0] == 0:
                image_clip = self.clip_preprocess(Image.open(black_path)).unsqueeze(0).to('npu')
            else:
                image_clip = self.clip_preprocess(clip_img).unsqueeze(0).to('npu')
...
class BaselineControlNet(nn.Module):
    ...
    def forward(self,x,x_t,image_clips,epoch):
        if epoch > opt.change_epoch:
            ...
            p = torch.full((x.size(0), 2, 1, 1, 1), 1).to('npu:0')
        x_copy = x
        x_t_copy = x_t
